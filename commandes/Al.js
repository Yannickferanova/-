...
yannick({nomCom:"bot",reaction:"🚀",categorie:"IA"},async(dest,zk,commandeOptions)=>{
...
yannick({ nomCom: "dalle", reaction: "🚀", categorie: "IA" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    let caption = '*Propulsé par YANNICK-MD*';
    
    if (data.status && data.owner && data.data) {
      // Utiliser les données retournées par le service
      const imageUrl = data.data;
      yk.sendMessage(dest, { image: { url: imageUrl }, caption: caption }, { quoted: ms });
    } else {
...
yannick({ nomCom: "gpt", reaction: "🚀", categorie: "IA" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;
